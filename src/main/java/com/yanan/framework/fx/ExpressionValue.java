...
//                	AnchorPane
						try {
							propertyModel = ReflectUtils.invokeMethod(namespace, key);
						} catch (IllegalAccessException | IllegalArgumentException | InvocationTargetException
								| NoSuchMethodException | SecurityException e) {
							throw new RuntimeException("could not found property for "+namespace.getClass().getName()+"."+key);
						}
                }
                if (propertyModel != null) {
    ...
    public Object getValue() {
    	Object value = expression.evaluate(namespace);
